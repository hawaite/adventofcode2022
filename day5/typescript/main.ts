...
class Command {
    private commandRegex:RegExp = new RegExp(/move (?<count>\d+) from (?<source>\d+) to (?<destination>\d+)/);
...
class State {
    ...
    constructor(initialStateRows: string[]){
        ...
        initialStateRows.reverse().forEach( value => {
            let parsedRow = lodash.chunk(value, stateColumnWidth).map(item => item[1]);
            parsedRow.forEach((item, ix)=>{ this.pushToColumn(ix + 1, item) })
        });
    }
    ...
    getTopOfEachStack = () => {
        let result:string = "";
        this.columnStateMap.forEach((col) => {result = result + col.at(-1)})
        return result;
    }
...
const main = () => {
    ...
    let lineList:string[] = fs.readFileSync("../input.txt", "ascii").split("\n");
    ...
    let startingState = lodash.takeWhile(lineList, line => line.trim() !== "" ).slice(0,-1);
    let commandList = lodash.dropWhile(lineList, line => !line.startsWith("move") );

    // init state
    let state:State = new State(startingState);
    let parsedCommandList:Command[] = commandList.map(cmdText => new Command(cmdText));
